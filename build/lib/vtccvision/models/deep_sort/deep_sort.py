...
def test():
    print("test deep sort")


class DeepSort(object):
    def __init__(self, model_path, use_cuda=True, set_para=False):
        ...
        if self.set_para == False:
            ...
            self.tracker = Tracker(
                metric, max_iou_distance=0.7, max_age=70, n_init=3
            )
    ...
    def update(self, bounding_boxs, scores, classId, ori_img):
        self.height, self.width = ori_img.shape[:2]

        xywh = torch.as_tensor(bounding_boxs)
        bbox_xywh = self._xyxy2xywh(xywh)
        classes = torch.as_tensor(classId)
        confidences = torch.as_tensor(scores)
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(
                np.array(
                    [x1, y1, x2, y2, track_id, class_id], dtype=np.int
                )
            )
        if len(outputs) > 0:
